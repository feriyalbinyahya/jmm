...
const NotifikasiInfoAkun = ({terpilih, setTerpilih, allChecked=false, setAllChecked, setJumlah, onPressNotif}) => {
    ...
    const getPesanDataOnRefresh = (page) => {
        ...
        NotifikasiServices.getNotifikasi("info akun", page)
        .then(res=> {
    ...
    const getDataPesan = () => {
        if(dataPesan.length ==0 ) setIsLoading(true);
        NotifikasiServices.getNotifikasi("info akun", currentPage)
        .then(res=>{
  ...
  return (
    <View style={{flex:1, backgroundColor: Color.neutralZeroOne}}>
        ...
        <View style={{alignItems: 'center', paddingVertical: '50%'}}>
            ...
            <Text style={{...FontConfig.titleTwo, color: Color.title}}>Yah, Belum ada info nih</Text>
            <Text style={{...FontConfig.bodyTwo, color: Color.secondaryText}}>Jika ada info akan tertampil disini</Text>
        </View> :
...
export default NotifikasiInfoAkun
